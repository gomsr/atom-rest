...
func Limiter(cacheType, limitTimeIP, limitCountIP int) gin.HandlerFunc {
	...
	if localLimiter == cacheType {
		return LocalLimiterV2(limitTimeIP, limitCountIP)
	}

	if redisLimiter == cacheType {
		return RedisLimiter(limitTimeIP, limitCountIP)
	}
